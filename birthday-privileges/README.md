...
- **Brand logo** — a small mark next to each row. Fetched at render time from DuckDuckGo's public icon service (`icons.duckduckgo.com/ip3/{domain}.ico`), keyed off the entry's `domain` field. No logo images are stored in this repo — nothing to source or license per brand. Google's favicon service was tried first but frequently fell back to a generic 16x16 icon for several of these exact domains (McDonald's, Major Cineplex, Sizzler, Café Amazon, MK Restaurant) — visually blank at that size; DuckDuckGo returned a real icon (32-128px+) for every domain checked, so it's the current default. Entries without a confirmed `domain` (or where the icon fails to load) fall back to a plain colored circle with the brand's first letter, rather than guessing a domain or leaving a broken image. See `BrandLogo` in `App.jsx`.
