      ...
      <div className={styles.header}>
        ...
        {hasData && (
          <div data-snapshot-hide>
            {/* Action area — 3-level hierarchy */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>

              {/* L1 — Primary: Save */}
              {onSave && (
                <button onClick={handleSave} style={{ width: '100%', padding: '10px', border: 'none', background: 'var(--color-accent)', color: 'var(--color-accent-text)', borderRadius: 8, cursor: 'pointer', font: 'inherit', fontWeight: 600, fontSize: 14 }}>
                  {t.saveBill}
                </button>
              )}

              {/* L2 — Secondary: Share + Line */}
              ...
              {/* L3 — Tertiary: New Bill + More (text-only, muted) */}
              <div style={{ display: 'flex', borderTop: '1px solid var(--color-border)', paddingTop: 4, marginTop: 2 }}>
                {onNewBill && (
                  <button onClick={onNewBill} style={{ flex: 1, padding: '5px 4px', border: 'none', background: 'transparent', color: 'var(--color-text-muted)', font: 'inherit', fontSize: 12, cursor: 'pointer', textAlign: 'center' }}>
                    + {lang === 'th' ? 'บิลใหม่' : 'New Bill'}
                  </button>
                )}
                <div style={{ position: 'relative' }} ref={moreRef}>
                  <button onClick={() => setMoreOpen(o => !o)} style={{ padding: '5px 8px', border: 'none', background: 'transparent', color: 'var(--color-text-muted)', font: 'inherit', fontSize: 12, cursor: 'pointer' }} aria-haspopup="true" aria-expanded={moreOpen}>
                    {t.more} ▾
                  </button>
                  {moreOpen && (
