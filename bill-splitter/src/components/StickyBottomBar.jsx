...
export default function StickyBottomBar({ memberCount, grandTotal, rawSubtotal, currencySymbol = '฿' }) {
  const { t } = useLang()
  ...
  const displayTotal = grandTotal > 0 ? grandTotal : (rawSubtotal ?? 0)
  if (displayTotal === 0 || isResultVisible) return null
  ...
  return (
    <button type="button" className={styles.bar} onClick={handleTap} aria-label={t.total || 'Total'}>
      ...
      <span className={styles.amount}>{currencySymbol}{displayTotal.toFixed(2)}</span>
