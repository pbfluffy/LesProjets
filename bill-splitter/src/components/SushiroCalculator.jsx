      ...
      <section className={styles.section}>
        ...
        <div className={styles.inputRow}>
          <input
            ...
            placeholder="ชื่อ เช่น พุม, กีกี้"
            className={nameError ? styles.inputError : ''}
          />
          <button type="button" className={styles.addBtn} onClick={handleAddPerson}>
            + เพิ่ม
          </button>
        </div>
            ...
            {store.people.map(name => (
              ...
              >
                ...
                <span
                  ...
                  onClick={e => { e.stopPropagation(); store.removePerson(name) }}
                >×</span>
              </button>
            ))}
          </div>
              ...
              return (
                <div key={plate.id} className={styles.plateRow}>
                  <span
                    ...
                    style={{ background: plate.color, border: `2px solid ${plate.border}` }}
                  />
                  ...
                  />
                  {count > 0 && (
                    <span className={styles.plateSubtotal}>฿{count * plate.price}</span>
                  )}
                </div>
              )
            })}
          ...
          <div className={styles.snackSection}>
            ...
            {(store.snacks[store.activePerson] ?? []).map(snack => (
              <div key={snack.id} className={styles.snackRow}>
                ...
                <button
                  type="button"
                  className={styles.snackRemove}
                  onClick={() => store.removeSnack(store.activePerson, snack.id)}
                >×</button>
              </div>
            ))}
        ...
        <section className={styles.section}>
          ...
          <div className={styles.toggles}>
            <label className={styles.toggle}>
              ...
              <span>VAT</span>
              <span className={`${styles.badge} ${styles.blue}`}>7%</span>
            </label>
            <label className={styles.toggle}>
        ...
        <section className={styles.section}>
                      ...
                      <div>
                        ...
                        <div className={styles.personPlateDots}>
                          {usedPlates.map(p => (
                            <span
                              key={p.id}
                              className={styles.plateDotSmall}
                              style={{ background: p.color, border: `1px solid ${p.border}` }}
                              title={`${p.label} ×${(store.plates[name] ?? {})[p.id]}`}
                            />
                          ))}
                          {usedPlates.length > 0 && (
                            <span className={styles.personPlateCount}>
                              {PLATES.reduce((s, p) => s + ((store.plates[name] ?? {})[p.id] ?? 0), 0)} จาน
                            </span>
                          )}
                          {personSnacks.length > 0 && (
                            <span className={styles.snackCount}>+ {personSnacks.length} รายการ</span>
                          )}
                        </div>
                      </div>
                    </div>
          ...
          <div className={styles.grandTotalBox}>
            <div className={styles.grandTotalRow}>
              <span className={styles.grandTotalLabel}>ยอดอาหาร</span>
              <span>฿{fmt(result.subtotal)}</span>
            </div>
            {result.serviceCharge > 0 && (
              <div className={styles.grandTotalRow}>
                <span className={styles.grandTotalLabel}>Service Charge (10%)</span>
                <span>฿{fmt(result.serviceCharge)}</span>
              </div>
            )}
            {result.vat > 0 && (
              <div className={styles.grandTotalRow}>
                <span className={styles.grandTotalLabel}>VAT (7%)</span>
