...
function setTask(name,taskObj,silent){
  ...
  if(!silent && !window.__CL){ logEv(DOGS[name].cv, `<b>${name}</b> — ${esc(taskText(taskObj))}`); bumpTasks(); }
...
function cycle(name){ if(window.__CL) return;
  if(!running) return;
...
/* ── run / pause ─────────────────────────────────────────────────────────── */
function setStatus(){ const w=running; const sv=$('statusVal'); if(sv) sv.textContent=w?tr('working'):tr('paused');
  const ss=$('statusSub'); if(ss) ss.textContent=w?tr('onshift'):tr('napping'); meta.textContent=w?tr('metaWorking'):tr('metaPaused'); }
function start(){
...
/* live work-log: GitHub commits changelog (dog voice) · BUG-20 */
async function loadChangelog(){
  var CACHE_KEY='studio_cl_v2',TTL=600000;
  function clEsc(s){return String(s).replace(/[&<>"']/g,function(m){return ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'})[m];});}
  var NOISE=[/^Add files via upload$/i,/^Delete /i,/^Merge /i,/^Initial commit$/i,/^Create .+ via upload$/i];
  var APP={pumgoda:'Pumgoda','bill-splitter':'Bill Splitter',nutritions:'Nutritions',nutrition:'Nutritions','nutritions-thailand':'Nutritions',studio:'Studio',shared:'Shared'};
